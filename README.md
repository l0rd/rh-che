...
### Build a rh-che Docker image that includes the assembly

```bash
# Make sure your shell is configured to reuse the Minishift docker daemon.
eval $(minishift docker-env)

# Pull upstream docker image
docker pull eclipse/che-server:5.18.0-centos
docker tag eclipse/che-server:5.18.0-centos eclipse/che-server:local

# Link the assembly generated by the maven build
rh_che_dir=$(pwd)
ln -f ${rh_che_dir}/assembly/assembly-main/target/eclipse-che-fabric8-1.0.0-SNAPSHOT.tar.gz  ${rh_che_dir}/dockerfiles/che-fabric8/eclipse-che.tar.gz

# Build Docker image
bash ./dockerfiles/che-fabric8/build.sh --organization:rhche --tag:local
```

### Run your locally built image on minishift

```bash
export CHE_OPENSHIFT_PROJECT=eclipse-che
export CHE_IMAGE_REPO=$(minishift openshift registry)/${CHE_OPENSHIFT_PROJECT}/che-server
export CHE_IMAGE_TAG=$(date "+%Y%m%d")
...
# Make sure the openshift project exist
oc new-project "${CHE_OPENSHIFT_PROJECT}"

# Tag and push the image to the minishift internal registry
docker tag rhche/che-server:local ${CHE_IMAGE_REPO}:${CHE_IMAGE_TAG}
docker push ${CHE_IMAGE_REPO}:${CHE_IMAGE_TAG}

# Run Che
bash ./dev-scripts/openshift_deploy.sh
```

### Run Che end to end tests for OSIO
...
# Trick that make as if oc is in current folder
ln $(which oc) $(pwd)/oc

# Run tests
export OSIO_URL=https://openshift.io
export TEST_SUITE=cheTest
...
export GH_USERNAME=<GitHub user linked with RHD account>
