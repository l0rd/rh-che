...
echo "VALIDATION: Validating image ${DOCKER_HUB_NAMESPACE}/che-server:${CHE_SERVER_DOCKER_IMAGE_TAG} =="
...
if [[ -z "$OSO_TOKEN" ]]; then
    echo "VALIDATION: Error OSO token is empty, cannot proceed with verification"
    exit 1
fi
echo "VALIDATION: Retrieved OSO token for user ${OSO_USER}"
...
echo "VALIDATION: Retrieved latest OSIO version to use for deployment: ${OSIO_VERSION}"
...
echo "VALIDATION: Cleaning-up previous deployment"
oc delete all -l project=che

echo "VALIDATION: Deploying che-server"
...
oc apply -f -
echo "VALIDATION: Applied http://central.maven.org/maven2/io/fabric8/online/apps/che/${OSIO_VERSION}/che-${OSIO_VERSION}-openshift.yml to ${OSO_API_ENDPOINT} project ${CHE_OPENSHIFT_PROJECT}"

## Check status of deployment
che_server_status=$(oc get pods | awk '{ if ($1 ~ /che-[0-9]+-.*/ && $1 !~ /che-[0-9]+-deploy/) print $3 }')
counter=0
timeout=240
echo "VALIDATION: Checking state of Che pod"
# Wait up to 1 minutes for running Che pod
while [ "${che_server_status}" != "Running" ]; do
    che_server_status=$(oc get pods | awk '{ if ($1 ~ /che-[0-9]+-.*/ && $1 !~ /che-[0-9]+-deploy/) print $3 }')
    counter=$((counter+1))
    if [ $counter -gt $timeout ]; then
        echo "VALIDATION: Che server is not running after ${timeout} seconds"
        exit 1
    fi
    sleep 1
done
echo "VALIDATION: Che pod is running!"
server_log=$(oc logs dc/che)
counter=0
timeout=180
echo "VALIDATION: Checking whether a Che server in Che pod has already started."
# Wait up to 1 minute for running Che server in pod 
while [[ $(echo "${server_log}" | grep "Server startup in" | wc -l) -ne 1 ]]; do
    server_log=$(oc logs dc/che)
    counter=$((counter+1))
    if [ $counter -gt $timeout ]; then
        echo "VALIDATION: Error, server log does not contain information about server startup. Timeouted after ${timeout} seconds."
        exit 1
    fi
    sleep 1
done
echo "VALIDATION: Che server is running!"

echo "VALIDATION: Verification passed. Pushing Che server image to prod registry."
